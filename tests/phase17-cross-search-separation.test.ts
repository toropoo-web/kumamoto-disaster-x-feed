...
describe("phase17 cross-search feed separation", function () {
  ...
  test("cross-search queries stay within API length limit", function () {
    ...
    queries.forEach(function (query) {
      ...
      assert.doesNotMatch(query.query, /熊本地震|給水|支援物資|炊き出し/);
    });
  });
  ...
  test("cross-search queries use municipality batches only", function () {
    const queries = buildCrossSearchQueries();
    assert.ok(queries.length >= 4);
    queries.forEach(function (query) {
      assert.ok(query.municipalityBatch.length > 0, `missing municipality batch: ${query.id}`);
    });
  });
