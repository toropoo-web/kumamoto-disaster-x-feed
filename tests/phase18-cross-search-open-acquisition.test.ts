...
describe("phase18 open cross-search acquisition", function () {
  ...
  test("includes open regional queries without disaster clause", function () {
    const queries = buildCrossSearchQueries();
    const openQueries = queries.filter(function (query) {
      return query.queryType === "OPEN";
    });
    assert.ok(openQueries.length > 0);
    openQueries.forEach(function (query) {
      assert.match(query.query, /-is:retweet lang:ja/);
      assert.doesNotMatch(query.query, /熊本地震/);
    });
  });
});
